...
public class Run {
	public static void main(String[] args) {
		 ...
		 * 내부클래스로 선언하여 사용한다.
		 * 코드가 굉장히 길어지고 파일이 많아지면 불편하니까 사용, 유지보수가 더 쉬움
		 * 
		 * 내부클래스의 종류?
		 * 1) 인스턴스 내부 클래스
		 * 2) 정적 내부 클래스
...
//		OuterClass.Inner oci = oc.new Inner();  => 이너클래스를 public으로 바꾸면 가능 하지만 
//												   이너클래스는 보통 private을 씀
		//oc.outerMethod();
...
//		t1.start();
		...
		// 3) 앞으로 자주 사용할 메서드들..
		// 	- Arrays.asList(T ...a) : ArrayList -> 크기가 고정된 arrayList
		// ...? 가변인자
		 ...
		 * 		사용가능한 변수 : 외부클래스의 필드 및 static 모두 사용 가능.
		 * 	2) 정적 내부 클래스
		 * 		구현위치 : 필드와 동일한 위치에 구현
		 * 		생성시기 :  프로그램 시작시 즉, 외부클래스와 무관하게 생성
		 * 		사용가능한 변수 : 외부클래스의 static변수만 사용가능.
		 * 	3) 지역 내부 클래스
		 * 		구현위치 : 메서드내부에 구현
		 * 		생성시기 : 메서드실행시 생성
		 * 		사용가능한 변수 : 외부클래스의 필드 및 static변수
		 * 	4) 익명 내부 클래스
