...
public class KakaoUserInfoResponse {
	 ...
	 * 	  hashCode등의 함수를 재정의항 내부에 기능을 추가하기도 한다.
	 * 
	 * 
	 * DTO(Data Transfer Object)
	 * 	- API서버간에 데이터 송수신 활용되는 클래스.
	 * 	- 어플리케이션의 비즈니스로직에 사용되는 VO클래스들과는 다르게 데이터 "송수신"목적으로 사용하기 때문에
	 * 	  불필요한 메서드나 상속이 존재하지 않는다. 오직 데이터 저장(set) 및 조회기능(get)의 메서드만 존재.
	 * 	- API서버에서 제공하는 응답데이터를 DTO클래스로 정의하고 사용한다.
	 * 	- 거대 API서버 같은 경우 굉장히 많은 기능(서비스)이 있고 기능마다 요구하는 주소, 및 요청변수, 응답 
	 * 	  데이터가 다양하기 때문에 굉장히 많은 DTO클래스가 나올 수 있다. 
	 * 	- 너무 많은 클래스가 있다면 유지보수 및 관리가 힘드므로 하나의 클래스 안에 정적내부클래스 여러개를 만드는
	 * 	  방식이 주로 사용된다.
	 * */
	private Long id;
	...
	public static class KakaoAccount{
		...
		private Boolean profile_iamge_needs_agreement;
	}
