...
public class StreamMethods {
	 ...
	 * 4) peek     : 중간확인 메서드. 현재 요소를 가져아서 원하는 작업처리를 함.
	 * 5) sort     : 정렬메서드
	 * ...
	 * 
	 * 최종연산 메서드들
	 * 1) Iterating    : forEach -> 요소를 단순 반복시키는 함수
	 * 2) Collecting   : collect(Collectors.?) -> Collectors에서 제공하는 정적 메소드를 매개변수로
	 * 					 받아 스트림의 요소를 "내가 원하는 자료구조로 모아서 반환"해준다
	...
	public static void main(String[] args) {
		// 중간처리 메서드들 + forEach
		// 1) distinct, filter : 중복값 제거 및 필터링 기능
		...
		// 2) collect(Collectors.toSet()) : 스트림의 결과값을 Set으로 반환
		...
		// 초기값 미지정시 스트림의 첫번째 요소가 초기값으로 자동지정된다.
