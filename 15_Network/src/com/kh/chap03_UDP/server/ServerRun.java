...
public class ServerRun {
	 ...
	 * 	  초점을 맞춘 프로토콜. 전달받은쪽에서 데이터가 제대로 전달되었는지 확인하거나 수신하지 않기때문에
	 * 	  데이터 손실이 발생할 수 있다.
	 * 	
	 * 	- 번거로운 확인절차를 모두 날려버림으로써 바른 데이터전송이 가능하다.	
	 * 	  실시간 스트리밍, 온라인게임, 유튜브 등에서 사용하는 프로토콜.
	 * 
	 * UDP의 특징
	...
	public static void main(String[] args) {
		...
		try {
			...
			byte[] receiveData = new byte[2048]; // 클라이언트가 전달한 데이터를 저장할 저장공간
